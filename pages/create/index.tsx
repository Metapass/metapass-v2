...
const Create: NextPage = () => {
  ...
  useEffect(() => {
    try {
      if (isConnected) {
        const provider = multichainProvider as ethers.providers.Web3Provider;
        ...
        txn.wait().then(async (res: any) => {
          ...
          if (event.fee == 0) {
            await axios({
              ...
              headers: {
                authToken: process.env.NEXT_PUBLIC_BICONOMY_DASH_API as string,
      ...
      try {
        ...
        txn.wait().then(async (res: any) => {
          ...
          if (event.fee == 0) {
            ...
            let roomLink = await axios.post('/api/createRoom' as string, {
              title: event.title,
              host: event.owner,
              contractAddress: child,
            });
            try {
              await contract.updateLink(child, roomLink.data.meetingLink);
            } catch (e) {
              console.log(e);
            }
          } else {
    ...
    if (wallet.publicKey && program) {
      ...
      if (hostExist == null) {
        try {
          ...
          const transactionInstruction1 = createInitializeHostInstruction(
            accounts1,
            args,
          );
          ...
          const transactionData = {
            createEventInfo: {
              ...
              isCustomSplToken: event.customSPLToken ? true : false,
              customSplToken: CST,
            },
          };
        ...
        try {
          ...
          let CST;
          if (event.fee === 0) {
            CST = new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v');
          } else {
          ...
          if (wallet.signTransaction) {
            ...
            console.log('Event created', `https://solscan.io/tx/${txid}`);
            setEventLink(
              window.location.origin + '/event/' + eventPDA.toString(),
            );
            setTxnId(txid);
